...
import pdb
from convs.convit import ConVit
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layer4(x)
        dim = x.size()[-1]
        convit_x = self.convit.forward_features(input)[-1].permute((0,2,1))
        convit_x = convit_x.reshape(shape=(*convit_x.shape[:2],int(math.sqrt(convit_x.shape[-1])+0.5),-1))
        convit_x = self.adapt_fc(F.interpolate(convit_x,x.shape[-2:]))
        x = x + 0.1 * convit_x
        res["featuremaps"] = torch.flatten(x,start_dim=2,end_dim=3).permute((0,2,1))
