...
def get_args_parser():
    ...
    parser.add_argument('--name', default='dytox_vit', help='Name to display for screen')
    ...
    # Resuming
    parser.add_argument('--resume', default='distill.pth', help='resume from checkpoint')
    ...
    parser.add_argument('--lm',default=7,type=int)
...
def main(args):
    ...
    num_all_classes = sum(args.increment_per_task)
    class_center = torch.zeros((num_all_classes,args.embed_dim),device=device)
    for task_id, dataset_train in enumerate(scenario_train):
        accumulative_class = [0]
        for x in args.increment_per_task:accumulative_class.append(accumulative_class[-1]+x)
        if args.max_task == task_id:
            print(f"Stop training sobecause of max task")
            break
        ...
        if args.resume:
            utils.load_first_task_model(model_without_ddp, loss_scaler, task_id, args, path='distill.pth')
        ...
        for j in range(0,-2,-1):
            if task_id+j>=0:
                taskcls.append((accumulative_class[task_id+j],accumulative_class[task_id+1+j],task_id+j))
        for epoch in range(initial_epoch, epochs):
            if args.distributed:
                loader_train.sampler.set_epoch(epoch)
            lam = 1
            weight_before = copy.deepcopy(model.state_dict())
            train_stats, cur_cc = train_one_epoch(
                ...
                sam=sam,
                loader_memory=loader_memory,
                pod=args.pod if task_id > 0 else None, pod_scales=args.pod_scales, cc = class_center, classes=taskcls
            )
            weight_after = copy.deepcopy(model.state_dict())
            new_params = {name:weight_before[name]*lam+weight_after[name]*(1-lam) for name in weight_before}
            model.load_state_dict(new_params)
            class_center[accumulative_class[task_id]:accumulative_class[task_id+1]] = cur_cc[accumulative_class[task_id]:accumulative_class[task_id+1]]
        ...
        if memory is not None and args.distributed_memory:
            ...
            if os.path.isdir(args.resume) and os.path.exists(task_memory_path) and 0:
    ...
    with open('{}.txt'.format(args.lm),'w')as f:
        print(statistics.mean(accuracy_list),accuracy_list,file=f)
    if args.name:
